...
void USART_Init(USART_Handle_t *pUSARTHandle)
{
	...
	pUSARTHandle->pUSARTx->CR1 = cr1;

	uint32_t cr2 = pUSARTHandle->pUSARTx->CR2;

	//No of STOP bits
	cr2 &= ~(0x03 << USART_CR2_STOP);
	cr2 |= (pUSARTHandle->USART_Config.USART_NoOfStopBits << USART_CR2_STOP);

	pUSARTHandle->pUSARTx->CR2 = cr2;

	uint32_t cr3 = pUSARTHandle->pUSARTx->CR3;
	// Flow control
	if(pUSARTHandle->USART_Config.USART_HWFlowControl == USART_HW_FLOW_CTRL_NONE)
	{
		cr3 &= ~(1 << USART_CR3_CTSE);
		cr3 &= ~(1 << USART_CR3_RTSE);
	}
...
uint8_t USART_SendDataIT(USART_Handle_t *pUSARTHandle,uint8_t *pTxBuffer, uint32_t Len)
{
	...
	if(txstate != USART_STATE_TX_BUSY)
	{
		pUSARTHandle->txLength = Len;
		pUSARTHandle->pTxBuffer = pTxBuffer;
		pUSARTHandle->TxBusyState = USART_STATE_TX_BUSY;
...
uint8_t USART_ReceiveDataIT(USART_Handle_t *pUSARTHandle,uint8_t *pRxBuffer, uint32_t Len)
{
	...
	if(rxstate != USART_STATE_RX_BUSY)
	{
		pUSARTHandle->rxLength = Len;
		pUSARTHandle->pRxBuffer = pRxBuffer;
		pUSARTHandle->RxBusyState = USART_STATE_RX_BUSY;
...
/*************************Check for TC flag ********************************************/
	...
	if(temp1 && temp2 )
	{
		...
		if ( pUSARTHandle->TxBusyState == USART_STATE_TX_BUSY)
		{
			...
			if(! pUSARTHandle->txLength )
			{
				...
				USART_ApplicationEventCallback(pUSARTHandle, USART_EVENT_TX_COMPLETED);
			}
		}
	}
...
/*************************Check for RXNE flag ********************************************/
	...
	if(temp1 && temp2 )
	{
		...
		//this interrupt is because of txe
		if(pUSARTHandle->RxBusyState == USART_STATE_RX_BUSY)
		{
			...
			if(! pUSARTHandle->rxLength)
			{
				...
				USART_ApplicationEventCallback(pUSARTHandle,USART_EVENT_RX_COMPLETED);
			}
		}
	}
